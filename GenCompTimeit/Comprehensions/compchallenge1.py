...
#Note the str(x) is to ensure all list generated list items are string to avoid error in further processing for mixed types
fizzbuzz = ["fizz buzz" if x % 15 == 0 else "fizz" if x % 3 == 0 else "buzz" if x % 5 == 0 else str(x) for x in
            range(1, 31)]
print(fizzbuzz)

for buzz in fizzbuzz:
    # centralizing all string in the list, to 12 chars padding with *
