...
def nested_comp():
    exits_to_destination_3 = [[(xit, locations[xit]) for xit in exits if loc in exits[xit].values()]
                              for loc in sorted(locations)]
    # print the result before returning (iterating over list)
    for x in exits_to_destination_3:
        pass

    return exits_to_destination_3

#Note that generator if faster but slower if you have to iterate over the list (trade off between speed and time)
def nested_gen():
    exits_to_destination_3 = ([(xit, locations[xit]) for xit in exits if loc in exits[xit].values()]
                              for loc in sorted(locations))
    # print the result before returning (iterating over list makes it slower in generator)
    for x in exits_to_destination_3:
        pass
